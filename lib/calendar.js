...
function getCredentials() {
    if (!_credentials) {
        if (process.env.GOOGLE_SERVICE_ACCOUNT_B64) {
            _credentials = JSON.parse(Buffer.from(process.env.GOOGLE_SERVICE_ACCOUNT_B64, 'base64').toString('utf8'));
...
/**
 * Returns 'HH:MM' in America/Los_Angeles from an ISO dateTime string.
 * @param {string} dateTimeStr
 * @returns {string} e.g. '08:00'
 */
export function toHHMM(dateTimeStr) {
    const parts = new Date(dateTimeStr)
        .toLocaleTimeString('en-US', { timeZone: 'America/Los_Angeles', hour: '2-digit', minute: '2-digit', hour12: false })
        .split(':');
